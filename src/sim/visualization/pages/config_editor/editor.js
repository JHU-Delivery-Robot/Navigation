...
class Robot extends Drawable {
    ...
    constructor(scene) {
        super(scene, "robot");
        this.position = { x: 100.0, y: 100.0 };
        this.angle = 90.0;
        this.rotation_handle = new Point(scene, this, this.handle_position(), 5);
    }
    ...
    onChildMove(point, child) {
        ...
        let snap_points = [0.0, 45.0, 90.0, 135.0, 180.0, 225.0, 270.0, 315.0];
        for (var i = 0; i < snap_points.length; i++) {
            if (nearly_equal(snap_points[i], this.angle)) {
                this.angle = snap_points[i];
                break;
            }
        }
...
class Point extends Drawable {
    ...
    draw() {
        ...
        ctx.beginPath();
        ctx.arc(this.point.x, this.point.y, this.size, 0, 2 * Math.PI, false);
        ctx.closePath();
        ctx.fill();
    }
}
...
class Waypoints extends Drawable {
    ...
    constructor(scene) {
        ...
        this.waypoints = [
            new Point(scene, this, { x: 200, y: 200 }, this.waypoint_size),
            new Point(scene, this, { x: 200, y: 500 }, this.waypoint_size),
        ];
    }
    ...
    contains(point) {
        ...
        return closest[1] < 3*this.waypoint_size;
    }
...
class ExtendPolygon extends Action {
    ...
    do() {
        ...
        if (distance_to_start < 3 * this.polygon.vertex_size) {
...
function onPointerDown(event) {
    let pointer = {
        x: parseInt(event.clientX - canvas.offsetLeft),
        y: parseInt(event.clientY - canvas.offsetTop),
    };
    ...
    if (target != null) {
        ...
        if (drawing_mode == Modes.Polygon) {
            ...
            let action = new ExtendWaypoints(scene, waypoints, waypoints.waypoints.length+1, pointer);
            scene.apply(action);
        }
    }
